...
export default function EmsForm() {
  ...
  const buildPdf = async (formData: FormData) => {
    const [consentBytes, anamnesisBytes] = await Promise.all([
      fetch(assetPath("consentimiento-ems.pdf")).then((response) => response.arrayBuffer()),
