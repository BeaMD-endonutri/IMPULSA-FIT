...
type Appointment = { id: number; starts_at: number; ends_at: number; status: string; bonus_name: string; notes?: string | null };
type ClientDetail = { client: ClientSummary; bonuses: Bonus[]; movements: Movement[]; appointments: Appointment[] };
...
export function ProfessionalPortal() {
  ...
  if (checking) return <main className="private-loading"><img src="/logo-impulsa-fit-transparent.webp" alt="IMPULSA FIT" /><span>Cargando acceso seguro…</span></main>;
  ...
  return <main className="private-app">
    ...
    <div className="private-shell">
      ...
      <section className="private-content">
        ...
        {tab === "calendar" && <AppointmentCalendar clients={clients} onBalancesChanged={() => void loadAll()} />}
...
function ClientRecord({ detail, busy, onBack, onSave, onCreateBonus, onQuickMovement, onCustomMovement, onResetPassword }: { detail: ClientDetail; busy: boolean; onBack: () => void; onSave: (event: FormEvent<HTMLFormElement>) => void; onCreateBonus: (event: FormEvent<HTMLFormElement>) => void; onQuickMovement: (id: number, delta: number, reason: string) => void; onCustomMovement: (event: FormEvent<HTMLFormElement>, id: number) => void; onResetPassword: () => void }) {
  ...
  return <><button className="record-back" onClick={onBack}><ArrowLeft size={17} /> Volver a clientes</button><div className="record-header"><div className="record-avatar"><CircleUserRound size={36} /></div><div><p>FICHA DE CLIENTE</p><h1>{client.first_name} {client.last_name}</h1><span className={`status-pill ${client.status}`}>{client.status === "active" ? "Cliente activo" : "Cliente inactivo"}</span></div><button onClick={onResetPassword}><KeyRound size={17} /> Nueva contraseña temporal</button></div><div className="record-layout"><div className="record-main"><section className="dashboard-panel"><div className="panel-title"><div><h2>Bonos y sesiones</h2><p>Añade el saldo cuando el cliente pague; la agenda descontará cada sesión.</p></div></div><div className="bonus-list">{detail.bonuses.length ? detail.bonuses.map((bonus) => <article className="bonus-card" key={bonus.id}><div className="bonus-card-top"><div><span>BONO ACTIVO</span><h3>{bonus.name}</h3><small>{formatDate(bonus.expires_at)}</small></div><div className="bonus-balance"><strong>{bonus.remaining_sessions}</strong><span>sesiones</span></div></div><div className="quick-movements bonus-payment-buttons"><button disabled={busy || bonus.remaining_sessions <= 0} onClick={() => onQuickMovement(bonus.id, -1, "Ajuste manual: sesión consumida")}><Minus size={17} /> Restar 1</button>{[1, 5, 10].map((amount) => <button key={amount} disabled={busy} onClick={() => onQuickMovement(bonus.id, amount, `Pago registrado: +${amount} ${amount === 1 ? "sesión" : "sesiones"}`)}><Plus size={17} /> {amount}</button>)}</div><form className="custom-movement" onSubmit={(event) => onCustomMovement(event, bonus.id)}><input name="delta" type="number" placeholder="Ej. -2 o 3" required /><input name="reason" placeholder="Motivo del ajuste" minLength={3} required /><button disabled={busy}>Registrar</button></form></article>) : <div className="private-empty">Este cliente todavía no tiene bonos.</div>}</div></section><section className="dashboard-panel"><div className="panel-title"><div><h2>Sesiones agendadas</h2><p>Historial permanente de citas del cliente.</p></div></div><div className="client-appointment-history">{detail.appointments?.length ? detail.appointments.map((appointment) => <div key={appointment.id}><CalendarClock size={18} /><div><strong>{new Intl.DateTimeFormat("es-ES", { weekday: "short", day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(appointment.starts_at))}</strong><p>{appointment.bonus_name}{appointment.notes ? ` · ${appointment.notes}` : ""}</p></div><span className={appointment.status}>{appointment.status === "completed" ? "Realizada" : appointment.status === "cancelled" ? "Cancelada" : "Programada"}</span></div>) : <div className="private-empty">Todavía no hay citas registradas.</div>}</div></section><section className="dashboard-panel"><div className="panel-title"><div><h2>Historial de movimientos</h2><p>Registro completo de altas, consumos y ajustes.</p></div></div><div className="movement-list">{detail.movements.length ? detail.movements.map((movement) => <div key={movement.id}><span className={movement.delta > 0 ? "positive" : "negative"}>{movement.delta > 0 ? `+${movement.delta}` : movement.delta}</span><div><strong>{movement.bonus_name}</strong><p>{movement.reason}</p></div><small>{formatDate(movement.created_at)}</small></div>) : <div className="private-empty">Sin movimientos registrados.</div>}</div></section></div><aside className="record-side"><form className="record-form" onSubmit={onSave}><div className="panel-title"><div><h2>Datos de la ficha</h2><p>Información privada y administrativa.</p></div><Pencil size={18} /></div><label>Nombre<input name="firstName" defaultValue={client.first_name} required /></label><label>Apellidos<input name="lastName" defaultValue={client.last_name} required /></label><label>Correo<input name="email" type="email" defaultValue={client.email} required /></label><label>Teléfono<input name="phone" defaultValue={client.phone || ""} /></label><label>Estado<select name="status" defaultValue={client.status}><option value="active">Activo</option><option value="inactive">Inactivo</option></select></label><label>Notas internas<textarea name="notes" rows={5} defaultValue={client.notes || ""} /></label><button className="private-primary-button compact" disabled={busy}>Guardar ficha</button></form><form className="new-bonus-form" onSubmit={onCreateBonus}><div className="panel-title"><div><h2>Nuevo bono</h2><p>Crea un bono base; después podrás sumar +1, +5 o +10.</p></div><PackagePlus size={20} /></div><label>Nombre del bono<input name="name" placeholder="Ej. Bono entrenamiento" required /></label><label>Número de sesiones<select name="sessions" defaultValue="5"><option value="1">1 sesión</option><option value="5">5 sesiones</option><option value="10">10 sesiones</option></select></label><label>Fecha de caducidad<input name="expiresAt" type="date" /></label><button className="private-primary-button compact" disabled={busy}>Añadir bono</button></form></aside></div></>;
}
